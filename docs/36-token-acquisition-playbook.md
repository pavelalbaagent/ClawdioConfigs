...
## Phase 1: Required for current VPS command center (`bootstrap_command_center` + `hybrid_124` + `addons_off`)
...
4. `GEMINI_API_KEY`
   ...
   - This powers the live low-cost and balanced chat lanes.

## Phase 2: Calendar and personal-task surface (part of `bootstrap_command_center`)
...
7. `PERSONAL_TASK_PROVIDER` and provider token
...
## Phase 3: Core integrations after command center
...
11. `AGENT_TASK_PROVIDER` and provider token
...
15. `OPENAI_API_KEY`
   ...
   - Needed for the active embedding-backed memory lane.
