...
| Multi-agent conversational runtime | done | done | done | partial | `assistant`, `researcher`, and `builder` now have bounded chat runtimes with checkpoints, selective memory, and explicit space routing. | Add a visible session registry and make `fitness_coach` a real specialist runtime. |
