...
| ResearchFlow orchestration | done | done | partial | partial | The researcher now has one orchestration layer wrapping the daily job-search digest and AIToolsDB AI-tools digest, with a shared status snapshot and dashboard visibility. | Add explicit runtime tests, deploy the wrapper services/timers, and verify both daily deliveries land in the research chat. |
